...
export function FilterBar(props: FilterBarProps) {
  return (
    <div className="filter-bar" aria-label="案例篩選">
      ...
      <label className="select-field"><span className="sr-only">滯留類別</span><select value={props.category} onChange={(event) => props.onCategoryChange(event.target.value)}><option value="">滯留類別</option>{props.categories.map((item) => <option key={item}>{item}</option>)}</select></label>
